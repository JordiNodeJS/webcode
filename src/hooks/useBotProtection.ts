...
export function useBotProtection(config: BotProtectionConfig = {}) {
  ...
  }, [honeypotValue, formStartTime, timeThreshold, submissionCount, maxSubmissions, isBlocked]);
  ...
  return {
    ...
    isBlocked,
    submissionCount,
    remainingCooldown: isBlocked ? Math.max(0, cooldownPeriod - (Date.now() - lastSubmission)) : 0
  };
}
