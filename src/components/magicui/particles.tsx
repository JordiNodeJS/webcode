...
export const Particles: React.FC<ParticlesProps> = ({
  ...
  const circleParams = useCallback((): Circle => {
    ...
    const alpha = 0;
    const targetAlpha = parseFloat((Math.random() * 0.3 + 0.2).toFixed(1));
    ...
    if (context.current) {
      for (let i = 0; i < circlesArray.length; i++) {
        for (let j = i + 1; j < circlesArray.length; j++) {
          const x1 = circlesArray[i].x + circlesArray[i].translateX;
          const y1 = circlesArray[i].y + circlesArray[i].translateY;
          const x2 = circlesArray[j].x + circlesArray[j].translateX;
          const y2 = circlesArray[j].y + circlesArray[j].translateY;
