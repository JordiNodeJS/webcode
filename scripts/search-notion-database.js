...
async function searchDatabase() {
  ...
  try {
    ...
    if (response.results.length === 0) {
      ...
      console.log("   1. La base de datos no está compartida con la integración");
    ...
    for (const db of response.results) {
      ...
      console.log("   Última edición:", new Date(db.last_edited_time).toLocaleString());
    ...
    if (allDatabases.results.length === 0) {
      console.log("   ⚠️ No hay bases de datos compartidas con esta integración\n");
    } else {
      for (const db of allDatabases.results) {
        console.log(`   - ${db.title?.[0]?.plain_text || "Sin título"} (${db.id})`);
      }
      console.log("");
    }
    ...
    if (error.code === "unauthorized") {
      ...
      console.log("   3. Genera una nueva clave en https://www.notion.so/my-integrations\n");
    } else if (error.code === "object_not_found") {
      console.log("\n💡 SOLUCIÓN:");
      console.log("   1. Comparte la base de datos con la integración");
      console.log("   2. En Notion: '...' → 'Add connections' → Selecciona tu integración\n");
    }
  }
}
