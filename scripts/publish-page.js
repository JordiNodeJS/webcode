...
async function publishPage() {
  ...
  try {
    ...
    const currentStatus = currentPage.properties.Status?.select?.name || "Sin status";
    ...
    console.log(`   Título: ${response.properties.Title?.title?.[0]?.plain_text || "Sin título"}`);
    ...
    if (error.code === "object_not_found") {
      ...
      console.log("   2. Asegúrate de que la página esté compartida con la integración\n");
    } else if (error.code === "validation_error") {
      console.log("\n💡 POSIBLE CAUSA:");
      console.log("   1. El campo 'Status' no existe en la página");
      console.log("   2. El valor 'Published' no existe como opción en el campo Status");
