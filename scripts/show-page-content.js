...
async function showPageContent() {
  ...
  try {
    ...
    for (let i = 0; i < response.results.length; i++) {
      ...
      console.log("\n" + "═".repeat(80));
      console.log(`📄 PÁGINA ${i + 1}: ${props.Title?.title?.[0]?.plain_text || "Sin título"}`);
      console.log("═".repeat(80));

      console.log("\n📋 METADATOS:");
      console.log(`   ID: ${page.id}`);
      console.log(`   Creada: ${new Date(page.created_time).toLocaleString()}`);
      console.log(`   Última edición: ${new Date(page.last_edited_time).toLocaleString()}`);
      ...
      console.log(`   📄 Description: "${props.Description?.rich_text?.[0]?.plain_text || ""}"`);
      console.log(`   📅 PublishedDate: ${props.PublishedDate?.date?.start || "null"}`);
      
      const tags = props.Tags?.multi_select?.map(t => t.name).join(", ") || "ninguno";
      console.log(`   🏷️  Tags: [${tags}]`);
      
      const author = props.Author?.people?.[0]?.name || props.Author?.people?.[0]?.id || "sin autor";
      ...
      try {
        ...
        if (mdString.parent && mdString.parent.trim().length > 0) {
          ...
          console.log(`   Longitud total: ${mdString.parent.length} caracteres`);
    ...
    for (const page of response.results) {
      ...
      try {
        ...
        const mdString = n2m.toMarkdownString(mdblocks);
        if (mdString.parent && mdString.parent.trim().length > 0) {
          withContent.push(page.properties.Title?.title?.[0]?.plain_text || "Sin título");
        } else {
          withoutContent.push(page.properties.Title?.title?.[0]?.plain_text || "Sin título");
        }
      } catch (error) {
        withoutContent.push(page.properties.Title?.title?.[0]?.plain_text || "Sin título");
      }
