...
function validateOffer(offer, index, errors, warnings) {
  ...
  if (!offer.hasMerchantReturnPolicy) {
    ...
    if (!policy["@type"] || policy["@type"] !== "MerchantReturnPolicy") {
      errors.push(
        `${prefix}hasMerchantReturnPolicy debe tener @type "MerchantReturnPolicy"`
      );
    }
    if (!policy.applicableCountry) {
      errors.push(
        `${prefix}hasMerchantReturnPolicy debe tener "applicableCountry"`
      );
    }
    if (!policy.merchantReturnDays) {
      errors.push(
        `${prefix}hasMerchantReturnPolicy debe tener "merchantReturnDays"`
      );
    }
  }
  ...
  if (!offer.shippingDetails) {
    ...
    if (!shipping["@type"] || shipping["@type"] !== "OfferShippingDetails") {
      errors.push(
        `${prefix}shippingDetails debe tener @type "OfferShippingDetails"`
      );
    }
  ...
  if (
    offer.seller &&
    (!offer.seller["@type"] || offer.seller["@type"] !== "Organization")
  ) {
...
function main() {
  ...
  if (totalErrors === 0) {
    ...
    console.log(
      "🎯 Los errores de hasMerchantReturnPolicy y shippingDetails deberían estar resueltos."
    );
  } else {
  ...
  console.log(
    "4. Revisar Search Console para confirmar que los errores desaparecen"
  );
}
