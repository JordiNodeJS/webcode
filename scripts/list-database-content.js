...
async function listDatabaseContent() {
  ...
  try {
    ...
    for (let i = 0; i < response.results.length; i++) {
      ...
      // Status
      if (props.Status) {
        console.log(`  Status: ${props.Status.select ? props.Status.select.name : 'null'}`);
      }
      ...
      if (props.Description && props.Description.rich_text) {
        const desc = props.Description.rich_text.map(t => t.plain_text).join('');
      ...
      if (props.Author && props.Author.people) {
        const authors = props.Author.people.map(p => p.name || p.id).join(', ');
