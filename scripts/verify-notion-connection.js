...
async function verifyNotionConnection() {
  ...
  if (!process.env.NOTION_DATABASE_ID) {
    ...
    log('   Solución: Agrega NOTION_DATABASE_ID a tu archivo .env.local', 'yellow');
    process.exit(1);
  }
  ...
  try {
    ...
    log(`   📄 Total de páginas encontradas: ${response.results.length}`, 'cyan');

    if (response.results.length === 0) {
      log('\n⚠️  La base de datos está vacía', 'yellow');
      log('   Solución: Añade algunos posts a tu base de datos "WebCode Blog"', 'yellow');
      return;
    }
    ...
    log(`   📝 Posts en borrador: ${response.results.length - publishedResponse.results.length}`, 'yellow');
    ...
    if (error.code === 'object_not_found') {
      log('\n💡 SOLUCIÓN:', 'yellow');
      log('   La base de datos NO está compartida con tu integración.', 'yellow');
      ...
      log('   📚 Guía completa: docs/NOTION-INTEGRATION-SETUP-GUIDE.md\n', 'blue');
