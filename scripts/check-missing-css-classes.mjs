...
function main() {
	...
	const totalUsedClasses = new Set();
	const totalDefinedClasses = new Set();
	...
	POTENTIALLY_MISSING_CLASSES.forEach((className) => {
		...
		if (isUsed && !isDefined) {
			console.log(
				`❌ CLASE FALTANTE: ${className} - Se usa pero no está definida`,
			);
	...
	const missingClasses = Array.from(totalUsedClasses).filter(
		(cls) =>
